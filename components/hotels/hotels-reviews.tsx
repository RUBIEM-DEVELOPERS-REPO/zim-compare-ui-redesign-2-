                ...
                {topRated.map(h => (
                    <div key={h.id} className="glass-card p-5 hover:border-amber-500/40 group">
                        <div className="flex items-start justify-between mb-3">
                            <p className="text-sm font-bold text-foreground group-hover:text-amber-500 transition-colors tracking-tight">{h.name}</p>
                        ...
                        <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mt-1 italic">{h.reviewCount} {t("stays.reviews")} · {h.city}</p>
                            ...
                            return (
                                <div key={r.id} className="glass-card p-5 group hover:border-primary/40">
                                    <div className="flex items-start justify-between mb-4">
                                        <div>
                                            <p className="text-xs font-black text-foreground uppercase tracking-widest">{r.author}</p>
                                            <p className="text-[10px] font-black text-primary uppercase tracking-tight mt-0.5">{hotel?.name}</p>
                                        </div>
                                        <div className="flex items-center gap-2 px-2 py-1 bg-muted/30 rounded-lg border border-white/5">
                                            ...
                                            <span className="text-[9px] font-black text-muted-foreground uppercase tracking-tighter">{new Date(r.date).toLocaleDateString()}</span>
                                        </div>
                                    </div>
                                    <div className="mb-4">
